...
func (h *Handler) PublishGig(c *gin.Context) {
	...
	if _, err = authz.Check(authz.CheckRequest{Action: "PublishGig", Resource: "gig", ResourceID: gig.ClientID, Role: currentUser.Role, UserID: currentUser.ID}); err != nil {
	...
	err = h.GigModel.UpdateStatus(gig.ID, "open")
	if err != nil {
